...
class CachesBallMethod {
public:
    // constructors
    CachesBallMethod(const GeometricQueries<DIM>& queries_);
    CachesBallMethod(const GeometricQueries<DIM>& queries_,
    ...
    void solve(const PDE<T, DIM>& pde,
               ...
               int nWalks, SamplePoint<T, DIM>& samplePt,
                std::vector<CachesBall<T, DIM>>& cachesBalls
            ) const;
...
protected:
    ...
    void estimateSolution(const PDE<T, DIM>& pde,
                          const WalkSettings& walkSettings,
                          int nWalks, SamplePoint<T, DIM>& samplePt,
                        std::vector<CachesBall<T, DIM>>& cachesBalls) const;
...
template <typename T, size_t DIM>
inline CachesBallMethod<T, DIM>::CachesBallMethod(const GeometricQueries<DIM>& queries_):
...
template <typename T, size_t DIM>
inline void CachesBallMethod<T, DIM>::solve(const PDE<T, DIM>& pde,
                                       const WalkSettings& walkSettings,
                                       int nWalks, SamplePoint<T, DIM>& samplePt
                                        , std::vector<CachesBall<T, DIM>>& cachesBalls) const
{
    if (samplePt.estimationQuantity != EstimationQuantity::None) {
        if (samplePt.estimationQuantity == EstimationQuantity::SolutionAndGradient) {
            ...
            // std::cout << "Estimating solution..." << std::endl;
            estimateSolution(pde, walkSettings, nWalks, samplePt, cachesBalls);
        }
    }
}
                                       ...
                                       std::function<void(int, int)> reportProgress) const
{
    ...
    if (runSingleThreaded || walkSettings.printLogs) {
        std::cout << "Running " << nPoints << " walks..." << std::endl;
        for (int i = 0; i < nPoints; i++) {
            solve(pde, walkSettings, nWalks[i], samplePts[i],cachesBalls);
...
template <typename T, size_t DIM>
inline void CachesBallMethod<T, DIM>::computeReflectingBoundaryContribution(const PDE<T, DIM>& pde,
                                                                       const WalkSettings& walkSettings,
                                                                       float starRadius, bool flipNormalOrientation,
                                                                       pcg32& sampler, WalkState<T, DIM>& state) const
{
...
template <typename T, size_t DIM>
inline void CachesBallMethod<T, DIM>::computeSourceContribution(const PDE<T, DIM>& pde,
                                                           const WalkSettings& walkSettings,
                                                           const Vector<DIM>& direction,
                                                           float intersectionDist, pcg32& sampler,
                                                           WalkState<T, DIM>& state) const
...
template <typename T, size_t DIM>
inline float CachesBallMethod<T, DIM>::computeWalkStepThroughput(const PDE<T, DIM>& pde,
                                                            const WalkSettings& walkSettings,
                                                            const WalkState<T, DIM>& state) const
{
...
template <typename T, size_t DIM>
inline bool CachesBallMethod<T, DIM>::applyWeightWindow(const WalkSettings& walkSettings,
                                                   pcg32& sampler, WalkState<T, DIM>& state,
                                                   std::queue<WalkState<T, DIM>>& stateQueue) const
{
...
template <typename T, size_t DIM>
inline WalkCompletionCode CachesBallMethod<T, DIM>::walk(const PDE<T, DIM>& pde,
                                                    const WalkSettings& walkSettings,
                                                    float distToAbsorbingBoundary,
                                                    float firstSphereRadius,
                                                    bool flipNormalOrientation,
...
template <typename T, size_t DIM>
inline T CachesBallMethod<T, DIM>::getTerminalContribution(WalkCompletionCode code,
                                                      const PDE<T, DIM>& pde,
                                                      const WalkSettings& walkSettings,
                                                      WalkState<T, DIM>& state) const
{
...
template <typename T, size_t DIM>
inline void CachesBallMethod<T, DIM>::estimateSolution(const PDE<T, DIM>& pde,
                                                  const WalkSettings& walkSettings,
                                                  int nWalks, SamplePoint<T, DIM>& samplePt,
                                                    std::vector<CachesBall<T, DIM>>& cachesBalls
                                                ) const
{
    ...
    CachesBall<T, DIM> cachesBall(samplePt.pt, samplePt.distToAbsorbingBoundary);
    ...
    for (int w = 0; w < nWalks; w++) {
        ...
        while (!stateQueue.empty()) {
            ...
            if (code == WalkCompletionCode::ReachedAbsorbingBoundary ||
                ...
                // std::cout << "Walk contribution: " << totalContribution << std::endl;
                // std::cout << state.throughput << " " << state.totalReflectingBoundaryContribution << " "<< state.totalSourceContribution << std::endl;
                // record the walk length
                samplePt.statistics.addWalkLength(state.walkLength);
                cachesPoint.value = totalContribution;
                success = true;
            }
        }
    ...
    //if(cachesBall.radius >= 0.1){
        cachesBalls.push_back(cachesBall);
    //}

}

template <typename T, size_t DIM>
inline void CachesBallMethod<T, DIM>::estimateSolutionAndGradient(const PDE<T, DIM>& pde,
                                                             const WalkSettings& walkSettings,
                                                             int nWalks, SamplePoint<T, DIM>& samplePt) const
{
