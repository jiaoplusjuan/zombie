                                                    ...
                                                    std::queue<WalkState<T, DIM>>& stateQueue) const
{
    ...
    while (distToAbsorbingBoundary > walkSettings.epsilonShellForAbsorbingBoundary) {
        // std::cout << "dist to absorbing boundary: " << distToAbsorbingBoundary << std::endl;
...
inline T WalkOnStars<T, DIM>::getTerminalContribution(WalkCompletionCode code,
                                                      ...
                                                      WalkState<T, DIM>& state) const
{
    // std::cout << "get terminal contribution" << std::endl;
    // std::cout << bool(code==WalkCompletionCode::ReachedAbsorbingBoundary) << std::endl;
    // std::cout << bool(code==WalkCompletionCode::TerminatedWithRussianRoulette) << std::endl;
    // std::cout << bool(code==WalkCompletionCode::ExceededMaxWalkLength) << std::endl;//
    if (code == WalkCompletionCode::ReachedAbsorbingBoundary &&
        !walkSettings.ignoreAbsorbingBoundaryContribution) {
        // std::cout << "check dirichlet" << std::endl;
    ...
    for (int w = 0; w < nWalks; w++) {
        ...
        while (!stateQueue.empty()) {
            ...
            if (code == WalkCompletionCode::ReachedAbsorbingBoundary ||
                ...
                T terminalContribution = getTerminalContribution(code, pde, walkSettings, state); // dirclet
