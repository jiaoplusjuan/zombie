...
{
    solution.resize(samplePts.size(), T(1.000001f));
    std::cout << "resize solution to " << solution.size() << std::endl;
               ...
               std::vector<T>& solution)
{
    if (solverType == "wost") {
        ...
        std::cout << "number of sample points: " << samplePts.size() << std::endl;
...
int main(int argc, const char *argv[])
{
    ...
    const std::vector<Vector3i>& absorbingBoundaryIndices = modelProblem.absorbingBoundaryIndices;
    const std::vector<Vector3i>& reflectingBoundaryIndices = modelProblem.reflectingBoundaryIndices;
    const std::vector<Vector3>& absorbingBoundaryPositions = modelProblem.absorbingBoundaryPositions;
    const std::vector<Vector3>& reflectingBoundaryPositions = modelProblem.reflectingBoundaryPositions;
    const std::pair<Vector3, Vector3>& boundingBox = modelProblem.boundingBox;
    const zombie::GeometricQueries<3>& queries = modelProblem.queries;
    const zombie::PDE<float, 3>& pde = modelProblem.pde;
    bool solveDoubleSided = modelProblem.solveDoubleSided;

    // create solve locations on a grid for this demo
    std::vector<Vector3> solveLocations;
    std::vector<DistanceInfo> distanceInfo;
    createGridPoints(outputConfig, boundingBox, solveLocations);
    computeDistanceInfo<3>(solveLocations, queries, solveDoubleSided, distanceInfo);

    // solve the model problem
    std::vector<float> solution;
    runSolver<float, 3>(solverType, solverConfig, absorbingBoundaryPositions, absorbingBoundaryIndices,
    ...
    std::cout << "max solution: " << max_solution << std::endl;
    std::cout << "solve locations size:b " << solveLocations.size() << std::endl;
    ...
    //col 3: z
    //col 4: solution
    std::ofstream outputFile("output.csv", std::ios::out);
    for (int i = 0; i < (int)solveLocations.size(); i++) {
        Vector3 bmin = modelProblem.boundingBox.first;
        if(bmin[0] == solveLocations[i][0] || bmin[1] == solveLocations[i][1] || bmin[2] == solveLocations[i][2])
            continue;
        outputFile << solveLocations[i][0] << "," << solveLocations[i][1] << "," << solveLocations[i][2] << "," << solution[i] << std::endl;
    }
    outputFile.close();
